...
trasition(英:过渡)可以让我们在网页中显示两个状态的变化过程

[demo](/assets/download/transition1.html)

上面这个圆设置了:hover属性和trasition:1s,这样从正常到hover状态之间的变化就会以动画的形式过渡

全过度:trasition: 过渡的时间 不需要过渡的属性 过渡的形式(默认全部属性同时开始改变)
...
[自动生成贝塞尔曲线参数](http://matthewlein.com/ceaser/)

如果我们需要进行多样式过度

trasition: 过渡的时间 延迟执行时间 参与过度的属性,...

{% highlight css %}
.box{
		trasition:1s width ease-out,2s 1s height ease-in-out, 3s background;
	}
{% endhighlight %}
...
obj.addEventListener('transitionend',function(){},false); 		/*其他标准浏览器*/
obj.addEventListener('webkitTransitionEnd',function(){},false); /*兼容webkit浏览器*/
{% endhighlight %}
