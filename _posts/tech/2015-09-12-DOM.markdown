...
以上除IE8 以下，所有的标准浏览器都支持

document.documentElement.clientWidth 返回根节点宽度 （可见区宽）
document.documentElement.clientHeight 返回根节点高度 （可见区高）

所有浏览器都支持，但是没有定义W3c标准的话，以上的属性返回的值会多出body的margin值

window.body.clientWidth 可见区域宽度
window.body.clientHeight 可见区域高度
