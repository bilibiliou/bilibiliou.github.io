...
```ts
 function isLegalIPWithPort(str: string): boolean {
  /**
   正则是这样的：
   ip 匹配段： ((25[0-5]|(2[0-4]|1\d|[1-9]|)\d)\.?\b){4}?
   \.?\b 是 每一组的以.间隔，一共有四大组 {4}
   每组有可能的数字进行穷举, 这个是规则组：
    a. 25[0-5] 即 250 - 255
    b. 2[0-4]\d 即 200 - 249
    c. 1\d\d 即 100 - 199
    ...
    端口号匹配段：\d{0,}
   */
  return Boolean(str) && /\b((25[0-5]|(2[0-4]|1\d|[1-9]|)\d)\.?\b){4}?\:\d{0,}\b/.test(str);
}
```